...
def create_briefing(location: str, video_id: str | None, persona: str, language: str) -> dict[str, Any]:
    ...
    return {"ai_response": response, "clean_text": response, "location": location, "language": language, "video": {"id": video.video_id, "title": video.title, "published_at": video.published_at, "channel_title": video.channel_title, "source": video.source, "url": f"https://www.youtube.com/watch?v={video.video_id}"}, "matches": matches, "transcript_segments": len(transcript), "is_live": False, "data_source": "IMD YouTube transcript + Gemini"}
