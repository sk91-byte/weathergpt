...
def generate_weather_response(
    ...
    prompt = (
        ...
        f"Use ONLY the JSON weather data below and do not add values that are absent. If recent route context contains nearby_places, use only those real mapped places for location recommendations; never invent a café, hotel, hospital, or other venue. "
...
def generate_follow_up_suggestions(
    ...
    prompt = (
        ...
        "Stay inside the WeatherGPT boundary: every question must be answerable from "
        "weather, forecast, route, mapped-place, alert, travel, farming, research, "
        "or WeatherGPT app-feature context. Do not suggest unrelated medical, legal, "
        "financial, entertainment, or general web questions. If mapped nearby places "
        "are supplied, ask only about those real places and never invent a venue. "
