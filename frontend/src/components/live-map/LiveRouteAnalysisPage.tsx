...
export const LiveRouteAnalysisPage: React.FC<LiveRouteAnalysisPageProps> = ({
  ...
  onAnalyzeAI, onOpenChat, onNearbyCategory
}) => {
        ...
        <section>
          ...
          <div className="space-y-3">
            {routes.map((route) => {
              ...
              return (
                <button type="button" key={route.id} onClick={() => onSelectRoute(route.id)} className={`w-full rounded-3xl border-2 p-4 text-left shadow-sm transition ${selected ? 'border-blue-500 ring-2 ring-blue-100' : 'border-slate-200 bg-white hover:border-blue-300'}`}>
                  ...
                  {selected && <div className="mt-3 flex gap-2"><span className="rounded-lg bg-blue-600 px-3 py-2 text-[11px] font-black text-white">SELECTED ROUTE</span><span className="rounded-lg bg-slate-100 px-3 py-2 text-[11px] font-bold text-slate-600">Live provider data</span></div>}
                </button>
              );
            })}
          </div>
