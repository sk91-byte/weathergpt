...
export async function apiResolveLocation(query?: string, latitude?: number, longitude?: number, onSlow?: () => void) {
  if (latitude !== undefined && longitude !== undefined) {
    const response = await fetchWithTimeout(`/location/reverse?latitude=${latitude}&longitude=${longitude}`, {}, 20000, onSlow);
    const data = await response.json();
    if (import.meta.env.DEV) {
      console.log('[apiResolveLocation reverse result]', data);
    }
    return data;
  }
  if (!query?.trim()) throw new Error('Please enter a location.');
  const places = await apiAutocompleteLocations(query);
...
export async function apiCalculateRoute(origin: ApiPoint, destination: ApiPoint, travelMode = 'driving', onSlow?: () => void): Promise<ApiRouteResponse> {
  ...
  return { ...payload, geometry: (payload.geometry?.coordinates || []).map((p: [number, number]) => [p[1], p[0]]), is_live: true, data_source: 'Render backend' };
}
...
export async function apiGetRouteWeather(routeOrGeometry: [number, number][] | string, _travelMode = 'driving', onSlow?: () => void): Promise<ApiRouteWeatherResponse> {
  ...
  const rainRisk = levels.includes('high') ? 'High' : levels.includes('moderate') ? 'Moderate' : levels.length ? 'Low' : 'Unavailable';
  return {
    safety_score: typeof overall.score === 'number' ? Math.round(overall.score) : null,
    rain_risk: rainRisk,
    waterlogging_risk: 'Unavailable',
    wind_risk: 'Unavailable',
    fog_risk: 'Unavailable',
    thunderstorm_risk: 'Unavailable',
    timeline: segments.map((s: any, i: number) => ({
      id: `segment-${i}`,
      name: `Route point ${i + 1}`,
      expected_time: s.start_time || 'Unavailable',
      distance_from_start_km: 0,
      weather_condition: s.weather?.condition || 'Unavailable',
      condition_icon: 'partly-cloudy',
      temp_c: Number(s.weather?.temperature_c || 0),
      feels_like_c: Number(s.weather?.temperature_c || 0),
      rain_prob: Number(s.weather?.rain_probability_percent || 0),
      rain_intensity: Number(s.weather?.rain_probability_percent || 0) >= 70 ? 'Heavy' : Number(s.weather?.rain_probability_percent || 0) >= 40 ? 'Moderate' : Number(s.weather?.rain_probability_percent || 0) > 0 ? 'Light' : 'None',
      precipitation_mm: Number(s.weather?.precipitation_mm || 0),
      waterlogging_risk: 'Unavailable',
      wind_speed_kmh: Number(s.weather?.wind_speed_kmh || 0),
      safety_score: typeof s.risk?.score === 'number' ? Math.round(s.risk.score) : 0,
      latitude: s.location?.latitude,
      longitude: s.location?.longitude
    })),
    risk_zones: [],
    is_live: Boolean(payload.data_available),
    source: payload.data_available ? 'Render backend' : 'Unavailable'
  };
}

export async function apiGetBestDepartureTime(routeId: string, _origin: ApiPoint, _destination: ApiPoint, currentSafetyScore: number | null, onSlow?: () => void): Promise<ApiBestDepartureTimeResponse> {
  const response = await fetchWithTimeout('/route/best-time', { method: 'POST', body: JSON.stringify({ route_id: routeId }) }, 30000, onSlow);
  const payload = await response.json();
  const options = (payload.alternative_times || []).map((x: any, i: number) => ({
    id: `departure-${i}`,
    title: x.departure_time === payload.recommended_departure_time ? 'Recommended' : 'Alternative',
    time: x.departure_time,
    safety_score: x.risk?.score ?? 0,
    is_recommended: x.departure_time === payload.recommended_departure_time,
    note: payload.reason || 'Backend forecast recommendation',
    rain_risk: x.risk?.level || 'Unavailable'
  }));
  const best = options.find((x: any) => x.is_recommended) || options[0];
  return {
    route_id: routeId,
    current_safety_score: currentSafetyScore,
    warning: (currentSafetyScore ?? 100) < 80,
    warning_message: (currentSafetyScore ?? 100) < 80 ? 'This route has some weather risk. Consider the recommended time.' : '',
    best_departure_time: payload.recommended_departure_time || '',
    best_option: best || { id: 'unavailable', title: 'Unavailable', time: 'Unavailable', safety_score: 0, is_recommended: false, note: payload.reason || 'Unavailable', rain_risk: 'Low' },
    options
  };
}

export async function apiGetRouteExplanation(routeId: string, _params?: any, onSlow?: () => void) {
  const response = await fetchWithTimeout(`/route/${encodeURIComponent(routeId)}/explanation`, {}, 20000, onSlow);
  const payload = await response.json();
  return { ...payload, explanation: Array.isArray(payload.explanation) ? payload.explanation.join(' ') : payload.explanation, factors: payload.explanation || [], safety_score: payload.risk_score };
}

export async function apiGetNearbyPlaces(latitude: number, longitude: number, radiusKm = 5, _category?: string, onSlow?: () => void): Promise<ApiNearbyPlacesResponse> {
  const response = await fetchWithTimeout(`/places/nearby?latitude=${latitude}&longitude=${longitude}&radius_km=${radiusKm}`, {}, 20000, onSlow);
  const payload = await response.json();
  return {
    places: (payload.places || []).map((p: any) => ({
      id: p.place_id,
      name: p.name,
      category: p.category || 'convenience',
      category_label: String(p.category || 'place').toUpperCase(),
      rating: 0,
      reviews: 0,
      distance_meters: Math.round((p.distance_km || 0) * 1000),
      walking_minutes: Math.max(1, Math.round((p.distance_km || 0) * 12)),
      address: p.formatted_address || p.address || 'Near route',
      latitude: p.latitude,
      longitude: p.longitude,
      open_status: 'Hours unavailable',
      shelter_feature: 'Nearby route place',
      route_relevance: 'Near selected route',
      is_live: true
    })),
    is_live: true
  };
...
export async function apiGetPointWeather(latitude: number, longitude: number, onSlow?: () => void): Promise<ApiPointWeatherResponse | null> {
  const response = await fetchWithTimeout(`/weather/current?latitude=${latitude}&longitude=${longitude}`, {}, 20000, onSlow);
  const payload = await response.json();
  const c = payload.current || {};
  return {
    latitude,
    longitude,
    location_name: 'Selected map location',
    temperature: Number(c.temperature_c || 0),
    feels_like: Number(c.apparent_temperature_c || c.temperature_c || 0),
    condition: c.condition || 'Current conditions',
    condition_icon: 'partly-cloudy',
    rain_probability: 0,
    current_precipitation: Number(c.rain_mm || c.precipitation_mm || 0),
    humidity: Number(c.humidity_percent || 0),
    wind_speed: Number(c.wind_speed_kmh || 0),
    wind_direction: 'Live',
    weather_source: payload.source || 'Render backend',
    is_live: true
  };
}
...
export async function apiGetLocationWeather(latitude: number, longitude: number, onSlow?: () => void) {
  if (import.meta.env.DEV) {
    console.log('[apiGetLocationWeather] Fetching for coords:', latitude, longitude);
  }
  const [weather, location] = await Promise.all([
    apiGetPointWeather(latitude, longitude, onSlow),
    apiResolveLocation(undefined, latitude, longitude, onSlow).catch((err) => {
      if (import.meta.env.DEV) {
        console.warn('[apiGetLocationWeather] reverse geocode error:', err);
      }
  ...
  if (import.meta.env.DEV) {
    console.log('[apiGetLocationWeather] Received location:', location, 'weather:', weather);
  }
